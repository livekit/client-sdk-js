...
describe('DataTrackDepacketizer', () => {
  ...
  it('should throw "unknownFrame" when a non single packet frame does not start with a "start" packet', () => {
    ...
    expect(() => depacketizer.push(packet)).toThrowError("Frame 103 dropped: Initial packet was never received.");
  });
  ...
  it('should throw "bufferFull" when too many packets have been sent', () => {
    ...
    const extraInterPacket = new DataTrackPacket(
      new DataTrackPacketHeader({ ...packetHeaderParams, marker: FrameMarker.Final, sequence: WrapAroundUnsignedInt.u16(DataTrackDepacketizer.MAX_BUFFER_PACKETS) }),
      packetPayload,
    );
    expect(() => depacketizer.push(extraInterPacket)).toThrowError("Frame 103 dropped: Reorder buffer is full.");
  });
});
