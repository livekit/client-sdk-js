...
export class DataTrackHandleError<Reason extends DataTrackHandleErrorReason = DataTrackHandleErrorReason> extends LivekitError {
  ...
  static tooLarge() {
    return new DataTrackHandleError("Value too large to be a valid track handle", DataTrackHandleErrorReason.Reserved);
  }

  static reserved(value: number) {
    return new DataTrackHandleError(`0x${value.toString(16)} is a reserved value.`, DataTrackHandleErrorReason.TooLarge);
  }
}
